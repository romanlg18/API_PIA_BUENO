...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 27-04-2023 a las 08:48:55
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `AgregarCliente` (IN `NombresP` VARCHAR(10000), IN `ApellidosP` VARCHAR(10000), IN `NumeroP` BIGINT(50), IN `CorreoP` VARCHAR(10000), IN `IdSegurosP` INT(10), IN `SeguroP` VARCHAR(50), IN `FECHAP` DATETIME)   BEGIN
	INSERT INTO args01.args_clientes(CL_NOMBRES, CL_APELLIDOS, CL_NUMERO, CL_CORREO, IDSEGUROS, SEGURO_INT, FECHA_ALTA) VALUES (NombresP, ApellidosP, NumeroP, CorreoP, IdSegurosP, SeguroP, FECHAP );
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SELECTCLIENTS` ()   BEGIN
	select IdCliente, CL_NOMBRES, CL_APELLIDOS, CL_NUMERO, CL_CORREO, IDSEGUROS, SEGURO_INT, ESTATUSCL, date_format(FECHA_ALTA, "%D/%M/%Y") FECHA_ALTA
	FROM ARGS_CLIENTES
